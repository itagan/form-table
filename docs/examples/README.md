...
| 组件诊断台 | `/debug` | 对比自定义组件直连和 FormTable 注册后的更新链路，并手动触发配置 warning。 |
...
| 配置 warning 是否触发 | `/debug` | 重复 key、缺失选项、slot/custom 必填项、rules 路径诊断 |
...
- 修复自定义组件协议或配置 warning 问题：优先补到组件诊断台。
