...
| Row props | `columns[].rowProps` | 当前单元格内唯一 Row，默认 `type: 'flex'` | ColumnContext + `row, index, displayIndex` |
...
`formItems[].meta` 是 FormTable 不求值的静态业务元数据，不接受动态回调。行级差异直接从上下文的 `row/value` 读取；需要切换整套业务注解时，由调用方提供另一份带稳定 `key` 的 Item 配置。
