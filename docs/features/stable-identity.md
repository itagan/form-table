...
FormTable 不持续扫描整张表和配置树。字段更新时会按需建立 `rowKey` 索引；身份缺失、重复或目标行已删除时直接忽略不确定的更新。开发环境还会在更新试图修改 `rowKey` 时给出警告。调用方仍需保证身份唯一、稳定；同级配置 `key` 由 TypeScript 结构约束、Vue 警告和业务测试负责检查。
