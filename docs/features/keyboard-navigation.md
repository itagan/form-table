...
打开[行列操作与异步提交 ↗](http://localhost:5173/row-column-operations)，点击姓名或评分输入控件后可以直接验证：

1. Enter 前进、Shift+Enter 后退。
2. 点击“姓名/评分换序”后，导航顺序随当前列顺序变化。
3. 点击“移除备注列”后，隐藏字段不再参与导航。
4. 勾选多行并点击“批量标记已复核”，可同时体验 `updateRows` 与键盘录入能力。
