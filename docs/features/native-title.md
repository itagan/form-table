...
`headerHint/hint` 保存提示内容和处理行为，整张 FormTable 通过 `hintOptions` 统一选择自动提示的展示策略。字符串默认由 FormTable 自动处理；默认 `title` 模式保持浏览器原生提示，`tooltip` 模式让默认表头、自定义表头 Slot 和全部字段外层共享一个 `el-tooltip`。
...
| 大量字段展示自身完整值 | `hintOptions.field: true` 或 formatter | 未声明 Hint 的字段自动继承统一格式化 |
| 提示依赖当前行、选项或权限 | `hint: context => ...` | 每次响应式更新都基于当前上下文求值 |
| 全表需要统一视觉和延迟 | `hintOptions.mode: 'tooltip'` | 表头和字段共享一个 Tooltip 实例 |
| 单个位置需要图标、富文本或独立交互 | `{ content, behavior: 'custom' }` + Slot | 内容保留在 Schema，展示完全交给调用方 |
...
推荐从最简单的 `hint: '...'` 开始。只有大量字段需要相同默认内容时才配置 `hintOptions.field`；只有需要统一视觉、键盘访问和延迟控制时才切换到 Tooltip；只有特殊交互才把 `behavior` 设为 `custom`。

## 核心模型

Hint 分成三个相互独立的决定：

1. **内容从哪里来**：固定字符串、动态回调，或表级 `field` 默认值。
2. **谁负责展示**：`behavior: 'auto'` 由 FormTable 展示；`behavior: 'custom'` 只保留标准化内容，由调用方展示。
3. **FormTable 如何展示**：整表统一选择浏览器原生 `title` 或单实例 `tooltip`。

因此 `hintOptions.field` 只负责生成默认字符串，不决定展示方式；`behavior` 只决定是否由 FormTable 自动处理；`mode` 只影响 `auto` Hint，不影响自定义 Hint。

```text
字段 hint / 表头 headerHint
        ↓ 解析内容
ResolvedFormTableHint { content, behavior }
        ↓ 判断 behavior
auto → 按整表 mode 输出 title 或 singleton Tooltip
...
| --- | --- | --- |
| 未声明或空 Hint，无全局 `field` | 保留 | 无 |
| 未声明或空 Hint，有全局 `field` 且产生内容 | 在渲染属性中被 Hint 取代 | 按整表 mode 展示 |
| `hint: false` | 保留 | 无 |
| `behavior: 'custom'` | 保留 | 无 |
| 有效 `auto` Hint | 在渲染属性中被 Hint 取代 | 按整表 mode 展示 |

记忆规则只有一句：不写或返回空值就继承全局，`false` 就关闭，提供内容就覆盖。没有有效 `auto` Hint 时，原始 title 始终透传；源配置对象从不修改。
...
`headerHint/hint` 表达提示语义；各级 `props.title` 只传给对应目标节点。有效 `auto` Hint 会在渲染属性中取代同层 `headerProps.title/formItemProps.title`；无有效 Hint、`behavior: 'custom'` 或 `hint: false` 时，原始 title 保持透传。
...
字符串等价于 `{ content: '提示内容', behavior: 'auto' }`。对象省略 `behavior` 时同样默认由 FormTable 托管：

```ts
hint: '字段说明'
// 等价于
hint: { content: '字段说明', behavior: 'auto' }
```

## 字段统一格式化

多数提示只是当前字段完整值时，可在表级配置一次默认值。未声明或返回空 Hint 的字段会自动继承：
...
| 返回值 | 行为 |
| --- | --- |
| 未声明、`null`、`undefined`、`''` | 回退 `hintOptions.field`；全局未配置则不显示 |
...
金额、日期和枚举等字段可以按 `itemConfig` 或 `fieldKey` 集中处理。配置 `field` 后普通字段无需逐项配置：
...
动态决定是否复用统一 formatter 时返回空值：

```ts
hint: ({ row }) => row.useCustomHelp ? '字段自己的说明' : null
```
...
如果只是希望 FormTable 在组件外层自动提示，保持默认 `behavior: 'auto'`，无需映射 props。只有业务组件内部需要消费内容时才做显式映射。
...
```ts
hint: ({ row }) => {
  if (!row.showHelp) return false
  if (row.useHelpIcon) {
    return { content: row.helpText, behavior: 'custom' }
  }
  return row.helpText
}
```

动态切换到 `custom` 后，FormTable 会停止自动提示并保留原始 `formItemProps`；切换为空值后回退全局，切换为 `false` 后明确关闭。Slot 和 component 配置在同一渲染周期读取到的是同一份标准化结果。

### 6. 表头、字段 Slot 与嵌套表格

- 默认表头和 `headerSlot` 都使用 `headerHint`；Slot 内只是增加图标或排版时，继续让 FormTable 托管。
- 图标本身需要成为独立触发点时，使用 `behavior: 'custom'`，并为图标提供 `tabindex`、`aria-label` 等键盘语义。
...
切换模式只改变 FormTable 托管 Hint 的呈现方式。`behavior: 'custom'` 的内容、Slot 内 Tooltip 和组件自己的 `title` 不受影响。
...
少量字段需要独立触发节点、内容或样式时，可以把内容继续保存在 Item `hint` 中，并用 `behavior: 'custom'` 将展示权交给字段 Slot：
...
`behavior: 'custom'` 对所有字段类型行为一致：内置字段、自定义组件和字段 Slot 都不会写入 title、内部标记或 ARIA，也不会占用表级 singleton。字段 Slot、component 动态配置和 listener 都能读取标准化后的 `{ content, behavior: 'custom' }`；业务组件本身不会被隐式注入同名 prop。表头也可用相同方式配置 `headerHint`，再从 `header.hint.content` 读取。
...
这种方式会按行、按字段创建 Tooltip 实例，适合少量特殊交互；大量普通说明仍应使用 FormTable 的表级单实例 Hint。如果提示内容完全不属于 Schema，也可以不配置 `hint`，直接在 Slot 内独立处理。完整对照可查看 [`/hint-scenarios`](http://localhost:5173/hint-scenarios)，其中包含内置字段、component、字段 Slot 与未配置 Hint 的行为矩阵及完整配置。
...
- `hintOptions.field` 只处理未声明或空 Hint，不会改写显式非空字符串或对象 Hint。
- `behavior: 'auto'` 时表头和字段 Slot 的外层提示由 FormTable 自动应用，Slot 内部不应重复创建 Tooltip；`behavior: 'custom'` 时 FormTable 不处理提示，与字段渲染类型无关。
