...
| `columns[].headerHint` | `DynamicValue<FormTableHint \| null \| undefined, ColumnContext>` | 可选 | `tableData, columnConfig` | 表头 Hint；`auto: true` 由 `hintOptions` 展示，`false` 时不自动处理 |
...
| `columns[].children[].children[].hint` | `DynamicValue<FormTableHint \| null \| undefined, ItemContext>` | 可选 | ItemContext | 字段 Hint；`auto: true` 应用于 `el-form-item`，`false` 时不自动处理 |
