...
`hintOptions` 是可辨识联合：title 模式使用 `{ mode?: 'title' }`，Tooltip 模式使用 `{ mode: 'tooltip', props?: ComponentProps }`。同一个 FormTable 只能选择一种展示方式；`props` 可配置 `placement/effect/openDelay/popperClass` 等 Element UI Tooltip 属性，`content/reference/popper/manual/value/enterable` 由内部单实例管理，不接受覆盖。完整行为见 [Hint 提示模式](../features/native-title.md)。
