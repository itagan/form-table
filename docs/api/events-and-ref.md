...
原生监听器同样在触发时取得当前字段上下文。事件目标是组件根节点；前后缀、清除按钮等内部节点的冒泡事件也可能到达监听器，需要时通过 `event.target` 判断。`listeners` 中的 `click.native` 不会转换为原生监听；`nativeListeners` 已包含 `.native` 语义，其中只写 `click`。组件事件、model、原生事件限制和 Slot 的完整选择规则见[字段组件事件与原生事件](../features/component-events.md)。
...
`updateRows([{ row, patch }])` 的事件顺序、失败边界和完整示例见[数据更新与受控回写](../features/data-updates.md#updaterows-原子更新多行)。行增删、复制和排序仍由页面直接替换 `tableData`。
