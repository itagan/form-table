...
原生监听器同样在触发时取得当前字段上下文。事件目标是组件根节点；前后缀、清除按钮等内部节点的冒泡事件也可能到达监听器，需要时通过 `event.target` 判断。声明式事件修饰符不属于配置 API。组件事件、model、原生事件和 Slot 的完整选择规则见[字段组件事件与原生事件](../features/component-events.md)。
