...
- 表头 Slot 获得原始 `columnConfig`、已解析的 `header.props` 与标准化 `header.hint`；`ownership: 'custom'` 时可自行消费提示。字段 component 动态配置、listener 和 Slot 同样获得标准化 `hint`。
- `colProps`、`formItemProps`、`component.props` 分别透传到对应 Element UI 层。
- `column.headerHint` 和 Item 的 `hint` 是语义提示入口；`hintOptions.field.enabled/formatter` 可让未声明 Hint 的字段统一格式化，字段用 `true` 强制启用、`false` 退出，显式内容保持最高优先级。字符串和 `ownership: 'table'` 对象由 `hintOptions` 统一选择原生 title 或表级单实例 Tooltip，`ownership: 'custom'` 时由调用方处理。Tooltip 模式支持表头键盘聚焦、Escape 关闭，并隔离嵌套 FormTable 实例。
