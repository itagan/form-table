...
- 表头 Slot 获得原始 `columnConfig`、已解析的 `header.props` 与标准化 `header.hint`；`auto: false` 时可自行消费提示。
- `colProps`、`formItemProps`、`component.props` 分别透传到对应 Element UI 层。
- `column.headerHint` 和 Item 的 `hint` 是语义提示入口；字符串和 `auto: true` 对象由 `hintOptions` 统一选择原生 title 或表级单实例 Tooltip，`auto: false` 时 FormTable 完全不处理提示，配置内容由调用方自行使用。
