<template>
  <div class="hotel-editor">
    <el-input :value="value.hotelName" placeholder="住宿名称" @input="patch('hotelName', $event)" />
    <el-input :value="value.roomName" placeholder="房间名称" @input="patch('roomName', $event)" />
    <el-input-number :value="value.roomCount" :min="1" controls-position="right" @input="patch('roomCount', $event)" />
    <el-select :value="value.roomType" placeholder="床型" @input="patch('roomType', $event)">
      <el-option label="大床" value="king" />
      <el-option label="双床" value="twin" />
      <el-option label="套房" value="suite" />
    </el-select>
    <el-input-number :value="value.guestCount" :min="1" controls-position="right" @input="patch('guestCount', $event)" />
    <el-select :value="value.breakfast" placeholder="早餐" @input="patch('breakfast', $event)">
