<template>
  <div class="transport-editor">
    <el-select v-if="demandType !== 'car'" :value="value.seatClass" :disabled="readonly" :placeholder="demandType === 'flight' ? '舱位等级' : '车次类型'" @input="patch('seatClass', $event)">
      <el-option v-for="option in seatOptions" :key="option.value" :label="option.label" :value="option.value" />
    </el-select>
    <el-select v-else :value="value.carType" :disabled="readonly" placeholder="车型" @input="patch('carType', $event)">
      <el-option label="轿车" value="sedan" />
      <el-option label="商务车" value="mpv" />
      <el-option label="大巴" value="bus" />
    </el-select>
    <el-input :value="value.departure" :disabled="readonly" placeholder="出发城市" @input="patch('departure', $event)" />
    <el-input :value="value.arrival" :disabled="readonly" placeholder="到达城市" @input="patch('arrival', $event)" />
...
import type { DemandDetail, TransportDemandType } from './types'

const props = defineProps<{ value: DemandDetail, demandType: TransportDemandType, readonly?: boolean }>()
const emit = defineEmits<{ (event: 'change', value: DemandDetail): void }>()
const seatOptions = computed(() => props.demandType === 'flight'
