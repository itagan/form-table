...
export const apiGroups: ApiGroup[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      { path: 'columns[].formItems[].component.model.valueToProp', type: '(context, bindingValue) => FormTableValue', defaultValue: '—', target: '组件 model prop', description: '同步转换字段值或 binding.map 组合值；context.value 保留主字段原值。', context: 'ItemContext + bindingValue' },
      { path: 'columns[].formItems[].component.model.valueFromEvent', type: '(context, ...eventArgs) => FormTableValue', defaultValue: '—', target: '字段写回值', description: '从 model 事件提取写回值；注册字段 type 会关联事件名及参数元组。', context: 'ItemContext + event args' },
