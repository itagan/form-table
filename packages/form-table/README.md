...
外层悬浮提示使用 `column.headerHint` 和 Item 的 `hint`。字符串与 `{ content, ownership: 'table' }` 默认由 FormTable 托管；`hintOptions.field.enabled` 可让未声明 Hint 的字段统一格式化当前值，字段用 `true` 强制启用、`false` 退出并保留底层属性。`hintOptions` 在整张表内统一选择 `{ mode: 'title' }`（默认）或 `{ mode: 'tooltip', props }`，后者让表头和所有 `el-form-item` 共享一个 `el-tooltip`。显式内容不经过 formatter；`ownership: 'custom'` 时 FormTable 不产生自动 DOM、ARIA 或 singleton 行为。component 动态配置、listener 和 Slot 都能读取标准化结果。
